...
func (tc *TodoCreate) SetComplete(b bool) *TodoCreate {
	tc.mutation.SetComplete(b)
	return tc
}

// SetNillableComplete sets the "complete" field if the given value is not nil.
func (tc *TodoCreate) SetNillableComplete(b *bool) *TodoCreate {
	if b != nil {
		tc.SetComplete(*b)
	}
	return tc
...
func (tc *TodoCreate) check() error {
	if _, ok := tc.mutation.Text(); !ok {
		return &ValidationError{Name: "text", err: errors.New(`codegen: missing required field "Todo.text"`)}
	}
